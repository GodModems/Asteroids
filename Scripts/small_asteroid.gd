...
func hit(damageN):
	health -= damageN
	if health <= 0:
		get_parent().update_score(get_meta("color"), 1)
		destroy()
...
func destroy():
	...
	var chance = randf()
	if color == Color("WHITE") and chance > 0.1:
		get_parent().spawn_item(position)
	if color == Color("RED") and chance > 0.2:
		get_parent().spawn_item(position)
	if color == Color("BLUE") and chance > 0.3:
		get_parent().spawn_item(position)
	queue_free()
